...
def find_center_mass(img, faces, heads):
    ...
    if debug:
        print("Image Dimensions: " + str(image_height) + ", " + str(image_width))
    ...
    for x, y, w, h in faces:
        ...
        y_total = (w*h) + y_total   # Calculate the area of all faces.
    ...
    # Compute the weighted average based on the size of the image.  An image full of faces will be 100%.
    y_weighted = (float(y_avg)/float((image_height*image_width))) * float(image_height)
    y_weighted_int = int(y_weighted)
	    
    if debug:
        print "x Avg: " + str(x_avg)
        print "y Weighted: " + str(y_weighted_int)

    MeanData(x_avg, y_weighted_int)

    return x_avg, y_weighted_int
